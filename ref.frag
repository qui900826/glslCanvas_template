...
float getDepth(vec2 pos, vec2 uv) {
    vec2 cellSmall = cellular(vec3(pos, u_time * .2));
...
void main(void)
{
    ...
    vec2 pos = uv * 3. + (noise(u_mouse.xy)*20. / u_resolution.xy); 
    // vec2 pos = uv * 3.;
    ...
    color += vec3(0.607,0.848,0.900)*1.5;

    // texture
    vec4 shadeColor= texture2D(u_tex0, st);
    
    gl_FragColor = vec4(color, bump.w * .8) * shadeColor;
}